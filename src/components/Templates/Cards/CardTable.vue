<template>
  ...
  <div
    ...
    :class="[color === 'light' ? 'bg-white' : 'bg-emerald-900 text-white']" v-for="user in info" :key="user.id"
  >
...
export default {
  ...
  mounted() {
    ApiService.get("/api/users")
      ...
      .catch(error => console.error('Erreur lors de la récupération des données dans CardTable.vue :', error));
 }
};
</script>
